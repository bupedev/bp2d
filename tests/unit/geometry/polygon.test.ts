import { Polygon, poly, LineSegment, lineSegment, Vertex, vtx, Vector, vec } from '../../../src/index';
...
function expectLineSegmentEquivalency(actual: LineSegment, expected: LineSegment): void {
...
describe('Edges Accessor', () => {
    it('should return a copy of the edges', () => {
        ...
        storedA[0] = lineSegment(vtx(0, 0), vtx(2, 2));
...
describe('Intersect LineSegment', () => {
    type TestObject = { polygon: Polygon, edge: LineSegment, intersections: Vertex[] };

    let boomerang = poly(testVertices.boomerang.slice());

    let cases: TestObject[] = [
        { polygon: boomerang, edge: lineSegment(vtx(0, 0), vtx(0, 0)), intersections: [] },
        { polygon: boomerang, edge: lineSegment(vtx(0, -1), vtx(0, 1)), intersections: [] },
        { polygon: boomerang, edge: lineSegment(vtx(0, -4), vtx(0, 4)), intersections: [vtx(0, -2), vtx(0, 2)] },
        { polygon: boomerang, edge: lineSegment(vtx(0, 4), vtx(0, -4)), intersections: [vtx(0, 2), vtx(0, -2)] },
        { polygon: boomerang, edge: lineSegment(vtx(0, -2), vtx(0, 2)), intersections: [vtx(0, -2), vtx(0, 2)] },
        { polygon: boomerang, edge: lineSegment(vtx(0, 2), vtx(0, -2)), intersections: [vtx(0, 2), vtx(0, -2)] },
        { polygon: boomerang, edge: lineSegment(vtx(0, 4), vtx(4, 4)), intersections: [vtx(2, 4), vtx(2.6, 4)] },
        { polygon: boomerang, edge: lineSegment(vtx(4, 4), vtx(0, 4)), intersections: [vtx(2.6, 4), vtx(2, 4)] },
        { polygon: boomerang, edge: lineSegment(vtx(-2, 1.5), vtx(4, 3)), intersections: [vtx(0, 2), vtx(2, 2.5)] },
        { polygon: boomerang, edge: lineSegment(vtx(4, 3), vtx(-2, 1.5)), intersections: [vtx(2, 2.5), vtx(0, 2)] },
        { polygon: boomerang, edge: lineSegment(vtx(0, 0), vtx(3, 5)), intersections: [vtx(3, 5)] },
        { polygon: boomerang, edge: lineSegment(vtx(0, 0), vtx(6, 10)), intersections: [vtx(3, 5)] },
        { polygon: boomerang, edge: lineSegment(vtx(0, 5), vtx(6, 5)), intersections: [vtx(3, 5)] },
        { polygon: boomerang, edge: lineSegment(vtx(3, 0), vtx(3, 10)), intersections: [vtx(3, 5)] },
        { polygon: boomerang, edge: lineSegment(vtx(-3, -1), vtx(4, 6)), intersections: [vtx(-2, 0), vtx(3, 5)] },
        { polygon: boomerang, edge: lineSegment(vtx(-2, 0), vtx(3, 5)), intersections: [vtx(-2, 0), vtx(3, 5)] },
        { polygon: boomerang, edge: lineSegment(vtx(-1, 1), vtx(2, 4)), intersections: [] },
        { polygon: boomerang, edge: lineSegment(vtx(2, -5), vtx(2, 5)), intersections: [vtx(2, -4), vtx(2, -2.5), vtx(2, 2.5), vtx(2, 4)] },
        { polygon: boomerang, edge: lineSegment(vtx(2, 5), vtx(2, -5)), intersections: [vtx(2, 4), vtx(2, 2.5), vtx(2, -2.5), vtx(2, -4)] },
    ];
    ...
    cases.forEach(testCase => {
        ...
        it(`should detect the intersections [${intersections}] for the polygon ${polygon} and the edge ${edge}`, () => {
            let actual = polygon.intersectLineSegment(edge);
...
describe('From Unordered Edges', () => {
    type TestObject = { edges: LineSegment[], polygon: Polygon };
    ...
    let cases: TestObject[] = [
        {
            edges: [lineSegment(sqv[0], sqv[1]), lineSegment(sqv[0], sqv[3]), lineSegment(sqv[2], sqv[1]), lineSegment(sqv[2], sqv[3])],
            polygon: poly([sqv[0], sqv[1], sqv[2], sqv[3]])
        },
        {
            edges: [lineSegment(twv[3], twv[4]), lineSegment(twv[1], twv[0]), lineSegment(twv[0], twv[5]), lineSegment(twv[5], twv[4]), lineSegment(twv[2], twv[1]), lineSegment(twv[2], twv[3])],
...
describe('Hatch Fill', () => {
    type TestObject = { polygon: Polygon, angle: number, spacing: number, jitter: (x: number) => number, edges: LineSegment[] };

    let cases: TestObject[] = [
        { polygon: poly(testVertices.square), angle: 0, spacing: 1, jitter: (x) => 0, edges: [lineSegment(vtx(-1, -1), vtx(1, -1)), lineSegment(vtx(-1, 0), vtx(1, 0)), lineSegment(vtx(-1, 1), vtx(1, 1))] },
        { polygon: poly(testVertices.square), angle: Math.PI / 4, spacing: 1, jitter: (x) => 0, edges: [lineSegment(vtx(Math.sqrt(2) - 1, -1), vtx(1, -Math.sqrt(2) + 1)), lineSegment(vtx(-1, -1), vtx(1, 1)), lineSegment(vtx(-1, Math.sqrt(2) - 1), vtx(-Math.sqrt(2) + 1, 1))] },
        { polygon: poly(testVertices.square), angle: Math.PI / 4, spacing: 2, jitter: (x) => 0, edges: [lineSegment(vtx(-1, -1), vtx(1, 1))] },
        { polygon: poly(testVertices.square), angle: Math.PI / 4, spacing: 1 / 2, jitter: (x) => 0, edges: [lineSegment(vtx(Math.sqrt(2) - 1, -1), vtx(1, -Math.sqrt(2) + 1)), lineSegment(vtx(-1 + Math.sqrt(2) / 2, -1), vtx(1, 1 - Math.sqrt(2) / 2)), lineSegment(vtx(-1, -1), vtx(1, 1)), lineSegment(vtx(-1, -1 + Math.sqrt(2) / 2), vtx(1 - Math.sqrt(2) / 2, 1)), lineSegment(vtx(-1, Math.sqrt(2) - 1), vtx(-Math.sqrt(2) + 1, 1))] },
        { polygon: poly(testVertices.square), angle: 0, spacing: 1, jitter: (x) => 0.5, edges: [lineSegment(vtx(-1, -0.5), vtx(1, -0.5)), lineSegment(vtx(-1, 0.5), vtx(1, 0.5))] },
        { polygon: poly(testVertices.square), angle: 0, spacing: 1, jitter: (x) => -0.5, edges: [lineSegment(vtx(-1, -0.5), vtx(1, -0.5)), lineSegment(vtx(-1, 0.5), vtx(1, 0.5))] },
        { polygon: poly(testVertices.farOffsetSquare), angle: 0, spacing: 1, jitter: (x) => 0.5, edges: [lineSegment(vtx(8, 8.5), vtx(10, 8.5)), lineSegment(vtx(8, 9.5), vtx(10, 9.5))] },
    ];
    ...
    cases.forEach(testCase => {
        ...
        it(`should be [${edges.join(", ")}] when the polygon is ${polygon}, angle is ${angle}, spacing is ${spacing} and jitter is ${jitter.toString()}`, () => {
            ...
            for (let i = 0; i < actual.length; i++) {
                expectLineSegmentEquivalency(actual[i], edges[i]);
            }
        });
    });
});
